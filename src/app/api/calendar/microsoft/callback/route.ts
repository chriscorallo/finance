...
import { exchangeMicrosoftCode, fetchMicrosoftAccountEmail } from "@/lib/calendar/microsoft-client";
...
export async function GET(request: NextRequest) {
  ...
  const expectedState = cookieStore.get("calendar_oauth_state_microsoft")?.value;
  cookieStore.delete("calendar_oauth_state_microsoft");

  if (oauthError || !code || !state || state !== expectedState) {
    return redirectToSchedule("/schedule?error=microsoft_connect_failed");
  }
  ...
  try {
    ...
    const { data: connection, error: connectionError } = await supabase
      ...
      .insert({
        ...
        display_name: email ? `Outlook — ${email}` : "Outlook Calendar",
        status: "active",
    ...
    if (connectionError || !connection) {
      return redirectToSchedule("/schedule?error=microsoft_save_failed");
    }
    ...
    await writeAuditEvent({
      ...
      eventData: { provider: "microsoft", connectionId: connection.id },
    });
  } catch {
    return redirectToSchedule("/schedule?error=microsoft_connect_failed");
  }
